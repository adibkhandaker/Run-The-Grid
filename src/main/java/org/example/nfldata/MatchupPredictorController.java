...
public class MatchupPredictorController {
    ...
    private void setupTeams() {
        // NFL Teams with their IDs and logos
        String[][] teams = {
    ...
    private void setupEventHandlers() {
        ...
        // Update logos when teams are selected
    ...
    private MatchupPrediction generatePrediction() throws Exception {
        ...
        // Fetch team statistics using the same API approach as TeamStatsController
        TeamStats team1Stats = fetchTeamStats(team1Id, year1);
        TeamStats team2Stats = fetchTeamStats(team2Id, year2);
        
        // Fetch team leaders for key player predictions
    ...
    private MatchupPrediction calculatePrediction(String team1Name, String team2Name, 
        ...
        // Calculate predicted scores using comprehensive algorithm
        ...
        // Predict key players (pass in predicted scores for consistency)
    ...
    private double calculateOffensiveRating(TeamStats stats) {
        ...
        // Points per game (40% weight) - Most important offensive stat
        rating += calculateRankBasedScore(stats, "Total Points Per Game", 0.4, false);
        
        // Total yards per game (25% weight)
        ...
        // Red zone touchdown percentage (15% weight)
    ...
    private double calculateDefensiveRating(TeamStats stats) {
        ...
        // For defensive stats, lower values are better (like points allowed), so we invert the ranking
        // Note: ESPN doesn't provide defensive stats in the offensive endpoint, so we'll use what we have
        
        // Sacks (40% weight) - higher is better for defense
        rating += calculateRankBasedScore(stats, "Sacks", 0.4, false);
        
        // Interceptions (30% weight) - higher is better for defense  
        rating += calculateRankBasedScore(stats, "Interceptions", 0.3, false);
        
        // Forced Fumbles (15% weight) - higher is better for defense
        rating += calculateRankBasedScore(stats, "Forced Fumbles", 0.15, false);
        
        // Passes Defended (15% weight) - higher is better for defense
    ...
    // Helper method to calculate score based on NFL ranking (1-32)
    private double calculateRankBasedScore(TeamStats stats, String statName, double weight, boolean lowerIsBetter) {
        ...
        try {
            // Extract rank number from rankDisplayValue (e.g., "5th" -> 5)
            ...
            // Convert rank to score (1st = 100, 32nd = 0)
            double rankScore;
            if (lowerIsBetter) {
                // For stats where lower values are better (like points allowed)
                rankScore = ((32 - rank) / 31.0) * 100.0;
            } else {
                // For stats where higher values are better (like points scored)
    ...
    private int calculateScore(double offense, double opponentDefense, double specialTeams) {
        // Calculate expected score based on team's offense vs opponent's defense
        // Offense and defense are now on 0-100 scales
        
        // Base score calculation: team's offense vs opponent's defense
        double offenseVsDefense = (offense - opponentDefense) / 10.0; // Scale to roughly -10 to +10
        
        // Convert to expected points (league average ~22 points)
        double baseScore = 22.0 + offenseVsDefense;
        
        // Factor in special teams (small impact)
        baseScore += (specialTeams / 100.0) * 3; // Max 3 point bonus for perfect special teams
        
        // Add realistic variance (±20% of base score)
        double variance = ThreadLocalRandom.current().nextDouble(-0.2, 0.2);
        int score = (int) Math.round(baseScore * (1 + variance));
        
        // Ensure realistic NFL score range
    ...
    private double calculateWinProbability(int score1, int score2, double off1, double def1, double spec1,
        ...
        // Secondary factor: overall team strength difference
        double team1Strength = (off1 + def1) / 2.0 + (spec1 * 0.1); // Offense + Defense main factors
        ...
        // Combine probabilities (80% score prediction, 20% overall strength)
    ...
    private PlayerPrediction predictTopPasser(String teamName, TeamLeaders leaders, TeamStats stats, int teamScore) {
        // Get the QB with the most passing yards (most likely the starter)
        String player = leaders.getTopLeader("Passing Yards");
        if (player == null) {
            // Try alternative passing categories
        ...
        // Use TEAM stats and scale them down to reasonable single-game numbers
        ...
        int yards = 225; // Default reasonable QB game
        double completion = 62.0; // Default completion %
        int tds = 1; // Default TDs
        
        try {
            if (passingYardsPerGame != null) {
                yards = (int) parseDouble(passingYardsPerGame);
                // Scale down slightly since this is team total, not individual QB
            ...
            if (completionPct != null) {
                completion = parseDouble(completionPct);
                    ...
                    // Estimate TDs based on team score (more realistic)
        // Assume ~1-2 field goals per game, so passing TDs = roughly 40-60% of total TDs
        int totalTDs = Math.max(0, (teamScore - 6) / 7); // Assume ~2 field goals per game
        tds = Math.max(0, Math.min(4, (int) (totalTDs * ThreadLocalRandom.current().nextDouble(0.4, 0.7))));
        
    } catch (NumberFormatException e) {}
    
    // Add realistic variance for individual game
    ...
    // TDs already calculated based on team score, just add small variance
    ...
    private PlayerPrediction predictTopRusher(String teamName, TeamLeaders leaders, TeamStats stats, int teamScore) {
        // Get the RB with the most rushing yards (most likely the starter)
        String player = leaders.getTopLeader("Rushing Yards");
        if (player == null) {
            // Try alternative rushing categories
        ...
        // Use TEAM rushing stats and scale down to reasonable single-game numbers
        String rushingYardsPerGame = stats.getStatValue("Rushing Yards Per Game");
        
        int yards = 95; // Default reasonable RB game
        int tds = 0; // Default TDs
        
        try {
            if (rushingYardsPerGame != null) {
                yards = (int) parseDouble(rushingYardsPerGame);
                // Scale down since this is team total, individual RB gets ~70% of team rushing
                yards = (int) (yards * 0.7);
            }
            
                    // Estimate TDs based on team score (more realistic) 
        // Rushing TDs = roughly 30-50% of total TDs
        int totalTDs = Math.max(0, (teamScore - 6) / 7); // Assume ~2 field goals per game
    ...
    private PlayerPrediction predictTopDefender(String teamName, TeamLeaders leaders, TeamStats stats) {
        // Try to get a defensive star - prefer sack leader, then tackle leader
        ...
        // Use realistic single-game defensive stats
        int tackles = ThreadLocalRandom.current().nextInt(4, 12);
        int gameSacks = 0;
        int gameInts = 0;
        
        // Randomly assign some big plays based on defensive strength
        ...
        try {
            if (teamSacks != null) {
                ...
                // Strong pass rush teams have higher chance of individual sacks
                if (seasonSacks > 35) {
            ...
            if (teamInts != null) {
                ...
                // Teams with more INTs have higher chance of individual picks
                if (seasonInts > 15) {
    ...
    private void displayKeyFactors(MatchupPrediction prediction) {
        ...
        // Add key factors based on the prediction
    ...
    private static class TeamLeaders {
        ...
        public void addLeader(String category, String shortName, String playerName, String displayValue, Double value) {
            // Store the top player for each category (only if not already stored - first one is the leader)
